...
/**
 * Locate the workflow list region by its accessible name. The list is only
 * rendered when there is at least one workflow.
 */
function workflowList(page: Page) {
  return page.getByRole('list', { name: copy.listAria });
}
...
/** Locate a row in the workflow list by its visible workflow name. */
function workflowRow(page: Page, displayName: string) {
  // The whole-row anchor's accessible name is `${openAction} ${name}`
  // (see WorkflowList.svelte). Using exact match avoids partial collisions
  // where one workflow name is a prefix of another.
  return page.getByRole('link', {
    name: `${copy.openAction} ${displayName}`,
    exact: true,
  });
}
...
test.describe('list-workflows: ユーザがディレクトリに置かれたワークフロー一覧を確認する', () => {
  ...
  test('ワークフローが 1 件もない状態でトップを開くと「No workflows yet」の empty state が表示される（不変条件 1: 0件はエラーにしない）', async ({
    ...
    // Assert: empty state のメッセージが両方とも表示されている
    const empty = emptyState(page);
    await expect(empty).toBeVisible();
    await expect(empty).toContainText(copy.emptyTitle);
    await expect(empty).toContainText(copy.emptyHint);

    // Assert: 一覧 (ul) はレンダリングされない
    await expect(workflowList(page)).toHaveCount(0);
  });
  ...
  test('複数のワークフローを置くと、ファイル名昇順で全件がリスト表示される（正常系: SummarizeEach + 不変条件 3 一意性）', async ({
    ...
    // Assert: 一覧 ul が描画されている / empty state は出ていない
    ...
    const links = page.getByRole('list', { name: copy.listAria }).getByRole('link');
  ...
  test('YAML が壊れていてもファイル名 (basename) で一覧に出る（不変条件 2: Name は必ず設定される）', async ({
    ...
    await expect(page.getByRole('list', { name: copy.listAria })).not.toContainText(
      'should-not-be-seen',
    );
  });
