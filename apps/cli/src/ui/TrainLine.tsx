...
// One full lap, regardless of scenery's trackLen — keeps apparent speed
// constant across scenery changes.
...
export function TrainLine(): ReactElement {
  const [now, setNow] = useState(() => Date.now());
  useEffect(() => {
    const id = setInterval(() => setNow(Date.now()), TICK_MS);
    return () => clearInterval(id);
  }, []);
  return <Text color={theme.accent}>{renderTrain(now)}</Text>;
}

function renderTrain(now: number): string {
...
function sceneryForLap(lap: number): Scenery {
  ...
  // tunnel laps. 12 slots — 0..5 plain, 6..7 sea, 8..9 mountain, 10..11 tunnel.
